    ...
    def read_window(
        ...
        *,
        ...
        with self.lock:
            ...
            injected_samples, synthetic_injection = _apply_synthetic_target5_test(
                trimmed.samples_2d,
                trimmed.timestamps_s,
                self.configured_sampling_rate,
                machine_id=machine_id,
            )
            signal = _select_axis(injected_samples, axis=axis)
            if signal.size < 32:
                raise ValueError(
            ...
            if synthetic_injection is not None:
                metadata.update(
                    {
                        "synthetic": True,
                        "data_provenance": "synthetic_injected_live_capture",
...
def _read_sdk_vibrometer_window_stateless(
    *,
    ...
    samples_2d, synthetic_injection = _apply_synthetic_target5_test(
        samples_2d,
        timestamps_s,
        configured_sampling_rate,
        machine_id=machine_id,
    )
