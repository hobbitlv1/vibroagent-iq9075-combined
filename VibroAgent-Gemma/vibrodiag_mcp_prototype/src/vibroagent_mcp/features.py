...
EPS = 1e-12
STRUCTURAL_FREQUENCY_MAX_HZ = 200.0
...
def compute_fft_features(signal_centered: np.ndarray, sampling_rate_hz: int, top_k: int = 5) -> dict[str, Any]:
    ...
    structural_indices = np.flatnonzero(
        (freqs > 0.0) & (freqs <= min(STRUCTURAL_FREQUENCY_MAX_HZ, sampling_rate_hz / 2.0))
    )
    if structural_indices.size:
        dominant_idx = int(structural_indices[np.argmax(mag_no_dc[structural_indices])])
