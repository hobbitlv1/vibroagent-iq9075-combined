...
def _sensor_config_from_dict(item: dict[str, Any], *, config_path: Path) -> SensorConfig:
    ...
    return SensorConfig(
        ...
        sensor_position=_normalize_position(item.get("sensor_position"), sensor_id=sensor_id),
        orientation_matrix=_normalize_orientation(item.get("orientation_matrix"), sensor_id=sensor_id),
    )
...
def _normalize_position(value: Any, *, sensor_id: str) -> tuple[float, float, float] | None:
    if value is None:
        return None
    if not isinstance(value, (list, tuple)) or len(value) != 3:
        raise ValueError(f"Sensor {sensor_id!r} sensor_position must contain three coordinates")
    position = tuple(float(item) for item in value)
    if not all(math.isfinite(item) for item in position):
        raise ValueError(f"Sensor {sensor_id!r} sensor_position must contain finite coordinates")
...
def _normalize_orientation(
    ...
    *,
    ...
    if value is None:
        return None
    if not isinstance(value, (list, tuple)) or len(value) != 3:
        raise ValueError(f"Sensor {sensor_id!r} orientation_matrix must be a 3x3 matrix")
    ...
    for row in value:
        ...
        if not all(math.isfinite(item) for item in converted):
            raise ValueError(f"Sensor {sensor_id!r} orientation_matrix must contain finite values")
