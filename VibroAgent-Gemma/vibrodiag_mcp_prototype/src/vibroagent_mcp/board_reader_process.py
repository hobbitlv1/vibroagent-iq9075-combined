...
class _BoardReaderClient:
    def __init__(self, *, key: tuple[str, str], ctx: mp.context.BaseContext, cpu_index: int):
        self.key = key
        self.ctx = ctx
    ...
    def ensure_started(self) -> None:
        with self._lock:
            ...
            self.process = self.ctx.Process(
                ...
                args=(self.key, self.cpu_index, self.request_q, self.response_q),
                daemon=True,
            )
            self.process.start()
...
def _get_client(key: tuple[str, str]) -> _BoardReaderClient:
    with _POOL_LOCK:
        ...
        if client is None:
            client = _BoardReaderClient(key=key, ctx=_get_context(), cpu_index=len(_POOL))
            _POOL[key] = client
        return client
