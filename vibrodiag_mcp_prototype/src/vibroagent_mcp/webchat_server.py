...
def _history_values_locked(sensor_id: str, metric_short: str) -> list[float]:
    ...
    values: list[float] = []
    for row in rows:
        if row.get("label") != _NORMAL_HISTORY_LABEL or row.get("q") or row.get("u"):
            continue
...
<body>
      ...
      <div class="graph-toolbar-inner">
        ...
        <details class="toolbar-more">
          ...
          <div class="controls">
            ...
            <button id="agentToggle" class="button" type="button">Agent On</button>
          </div>
        </details>
      </div>
    </section>
  ...
  <script>
    ...
    function updateAgentMonitor(data) {
      ...
      els.agentToggle.title = data.cached
        ? `Agent checked recently: ${status}`
        : `Agent checked: ${status}`;
      els.agentToggle.textContent = data.anomaly && popup.show_popup
        ? "Agent Alert"
        : "Agent Normal";
